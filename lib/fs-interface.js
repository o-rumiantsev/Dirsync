    ...
    const fetchFilesData = cb =>
      metasync.map(
        ...
        (err, filesdata) => {
          ...
          const files = new Map(filesdata);
          cb(null, { files });
        }
      );

    const fetchChildren = cb =>
      metasync.map(directories, readFileSubsystem, (err, nodes) => {
        if (err) {
          cb(err);
          return;
        }
...
const replacePath = (p, targetSubpath, sourceSubpath) => {
  if (sourceSubpath === './') return path.join(targetSubpath, p);
...
const buildNode = (node, targetPath, sourcePath, cb) => {
  ...
  const files =
    Array.from(node.files.entries(), ([name, data]) => {
